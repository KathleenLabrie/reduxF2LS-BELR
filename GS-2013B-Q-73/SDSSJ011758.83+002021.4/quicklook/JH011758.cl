...
# Science Dataset:
#
# Observation UT date : 2013 Oct 15
# Data filename prefix: S20131015S
# File numbers:
#      Science                    : 38-41   (JH, JH, 2pix-slit, 90s)
#      Darks for science          : 159-165 (90s)
#      Flat                       : 43      (JH, JH, 2pix-slit, 8s)
#      Darks for flat             : 180-186 (8s)
#      Arc                        : 44      (JH, JH, 2pix-slit, 15s)
#      Darks for arc              : 166-172 (30s, ** no 15s darks available)
#
# Telluric Dataset:
#
# Observation UT date : 2013 Oct 02
# Data filename prefix: S20131015S
# File numbers:
#      Telluric (HIP 6061)        : 23-26   (JH, JH, 2pix-slit, 2s)
#      Darks for telluric         : 152-158 (2s)
#      Flat                       : 43      (JH, JH, 2pix-slit, 8s)
#      Darks for flat             : 180-186 (8s)
#      Arc                        : 44      (JH, JH, 2pix-slit, 15s)
#      Darks for arc              : 166-172 (30s, ** no 15s darks available)
...
#     ...    print "JH011758: Deleting contents of %s" % (f2.database)
#     ...    iraf.delete (f2.database + "*", verify=no)

if (access(f2.database)) {
    printf ("JH011758: Deleting contents of %s\n", f2.database)
...
rawdir = "../../../raw/"
printf ("JH011758: Raw data is located in %s\n", rawdir)
...
# their own dataset.
print ("JH011758: Creating the reduction lists")
gemlist "S20131015S" "43"      > "flat.lis"
gemlist "S20131015S" "180-186" > "flatdark.lis"
gemlist "S20131015S" "44"     > "arc.lis"
gemlist "S20131015S" "166-172" > "arcdark.lis"
gemlist "S20131015S" "38-41" > "obj.lis"
gemlist "S20131015S" "159-165" > "objdark.lis"
gemlist "S20131015S" "23-26"   > "tel.lis"
gemlist "S20131015S" "152-154,156-158"     > "teldark.lis"  # 155 has bad section
...
#
#  KL: f2prepare divides by number of non-destructive reads, which is equal to 
#	LNRS keywork value.  LNRS comment says non-destructive read pairs.  What
#	does that mean?
#	Why is NREAD != LNRS ?  eg. Faint -> NREADS = 8 (OT agrees) but LNRS=6
#	Is f2prepare dividing by the right number?
#	Is this at all scientifically correct?
#
...
#
# KL: what is the fit supposed to look like?  Is supposed to fit the wiggles?
#	Or is it rather supposed to fit the overall shape?
#
# KL: I hit 'd' a couple times, then 'f', and the fit becomes a straight
#	horizontal line!
#
...
# there are dark current features (bright lines) that are in the tellurics 
# but no in the darks.
...
###############################################################################
#
# KL: For faint target the fl_cross=yes does not work well.
...
nsextract ("tfobj_comb.fits", fl_apall=yes, fl_findneg=no, fl_inter=yes, \
           fl_trace=yes)
# !!!!  This step hangs every time!  I need to kill the pyraf session,
# !!!!  (with kill -9!), start a fresh one, reset the environment, and
# !!!!  do the nsextract again.  Then it works.
